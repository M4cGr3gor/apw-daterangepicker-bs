...
@Component({
	selector: 'apw-daterangepicker-bs',
	styleUrls: ['./apw-daterangepicker-bs.component.scss'],
	templateUrl: './apw-daterangepicker-bs.component.html',
	host: {
	...
	providers: [
		{
			...
			useExisting: forwardRef(() => ApwDaterangepickerBsComponent),
			multi: true
		}
	]
})
export class ApwDaterangepickerBsComponent implements OnInit, OnChanges {
	public $event: any;
	public chosenLabel?: string;
	public calendarVariables: { left: any; right: any } = { left: {}, right: {} };
	public timepickerVariables: { left: any; right: any } = {
		left: {},
		right: {}
	};
	...
	@Input() set minDate(value: dayjs.Dayjs | string) {
		// accessors
		...
		for (
			...
			if (
				...
				calendar[row][col].format('YYYY-MM-DD') ===
					this.getMinDate()?.format('YYYY-MM-DD') &&
			...
			if (
				this.getMaxDate() &&
				calendar[row][col].format('YYYY-MM-DD') ===
					this.getMaxDate()?.format('YYYY-MM-DD') &&
	...
	updateMonthsInView() {
		...
		if (
			...
			this.linkedCalendars &&
			!this.singleDatePicker &&
			this.rightCalendar.month > <Dayjs>this.getMaxDate()
		) {
			this.rightCalendar.month = this.getMaxDate()?.clone().date(2);
			this.leftCalendar.month = this.getMaxDate()
				?.clone()
				.date(2)
				.subtract(1, 'month');
		}
	...
	clickDate(e: any, side: SideEnum, row: number, col: number) {
		...
		if (
			(this.endDate ||
				(date.isBefore(this.startDate, 'day') && !this.customRangeDirection)) &&
			!this.lockStartDate
		) {
			// picking start
			this.applyBtn.disabled = true;
		...
		for (let row = 0; row < 6; row++) {
			...
			for (let col = 0; col < 7; col++) {
				...
				if (
					...
					(calendar[row][col] < <Dayjs>this.endDate ||
						(calendar[row][col] < <any>this.nowHoveredDate &&
							this.pickingDate)) &&
