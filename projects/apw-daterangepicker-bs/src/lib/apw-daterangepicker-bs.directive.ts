...
import { LocaleConfig } from './apw-daterangepicker-locale.config';
import { ApwDaterangepickerLocaleService } from './apw-daterangepicker-locale.service';

@Directive({
	selector: 'input[apwDaterangepickerBs]',
	host: {
		'(click)': 'open()',
		'(keyup.esc)': 'hide()',
		'(blur)': 'onBlur()',
	...
	providers: [
		{
			...
			useExisting: forwardRef(() => ApwDaterangepickerBsDirective),
			multi: true
		}
	]
})
export class ApwDaterangepickerBsDirective
	implements OnInit, OnDestroy, OnChanges, AfterViewInit, AfterViewChecked
{
	public $event: any;
	public daterangepicker: ApwDaterangepickerBsComponent | any;
	...
	constructor(
		...
		private _localeService: ApwDaterangepickerLocaleService
	) {
		this.endKey = 'endDate';
		...
		this.daterangepickerRef = this.viewContainerRef.createComponent(
			ApwDaterangepickerBsComponent,
			{ injector: this.injector }
		);
		this.daterangepickerElement = (
			this.daterangepickerRef.hostView as EmbeddedViewRef<any>
		...
		this.daterangepicker = <ApwDaterangepickerBsComponent>(
			this.daterangepickerRef.instance
		);
	...
	ngOnInit() {
		...
		this.daterangepicker.startDateChanged
			.asObservable()
			.subscribe((itemChanged: any) => {
				this.startDateChanged.emit(itemChanged);
			});
		this.daterangepicker.endDateChanged
			.asObservable()
			.subscribe((itemChanged: any) => {
		...
		this.daterangepicker.firstDayOfNextMonthClass =
			this.firstDayOfNextMonthClass;
		this.daterangepicker.lastDayOfPreviousMonthClass =
	...
	ngOnDestroy() {
		this._resizeObserver?.unobserve(
			this.daterangepicker.pickerContainer.nativeElement
		);
		const reflectComponent = reflectComponentType(
			ApwDaterangepickerBsComponent
		);
	...
	setPosition() {
		...
		if (this.opens === 'center') {
			style = {
				...
				left:
					inputOffset.left +
					(inputOffset.width - pickerContainer.offsetWidth) / 2 +
					'px'
			};
		}
		if (this.opens === 'left') {
			style = {
				top: containerTop,
				right:
					window.innerWidth - (inputOffset.left + inputOffset.width) + 'px',
				bottom: containerBottom,
				left: 'auto'
			};
		}
	...
	outsideClick(event: any): void {
		if (
			!event.target ||
			event.target.classList.contains('apw-daterangepicker-action')
		)
			return;
